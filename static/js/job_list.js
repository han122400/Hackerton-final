...
function createJobCard(job) {
    ...
    card.className = 'bg-card border border-border rounded-lg p-5 cursor-pointer hover:shadow-md transition-shadow';
    card.addEventListener('click', () => selectJob(job));
    
    card.innerHTML = `
        <h3 class="font-semibold mb-2">${job.title}</h3>
        <p class="text-sm text-muted-foreground mb-3">${job.company} · ${job.location} · ${job.experience}</p>
        <div class="flex flex-wrap gap-2 mb-3">
            ${job.tags.map(tag => `<span class="bg-secondary text-secondary-foreground px-2 py-1 rounded text-xs">${tag}</span>`).join('')}
        </div>
...
function updateActiveFilters() {
    ...
    activeFiltersContainer.innerHTML = activeFilters
        .map(filter => `<span class="bg-primary text-primary-foreground px-2 py-1 rounded text-xs">${filter}</span>`)
        .join('');
}
