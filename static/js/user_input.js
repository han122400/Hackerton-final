...
function handleFormSubmit(e) {
  ...
  // OpenRouter에 "면접 시작" 텍스트 전송
  sendInterviewStart(interviewData)
}

// OpenRouter에 "면접 시작" 전송 후 3초 뒤 JSON 데이터 전송
async function sendInterviewStart(interviewData) {
  try {
    // 1단계: "면접 시작" 텍스트 전송
    console.log("OpenRouter에 '면접 시작' 전송...")
    const startRes = await fetch('/api/generate-text', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ prompt: '면접 시작' }),
    })
    ...
    setTimeout(async () => {
      try {
        const jsonPrompt = JSON.stringify({
          name: interviewData.userName || '지원자',
          position: interviewData.role,
          company: interviewData.company,
          education: '대학교 졸업',
          experience: interviewData.resume_summary_docx,
          skills: interviewData.extra_info,
          jobDetail: interviewData.jobDetail,
        })

        const jsonRes = await fetch('/api/generate-text', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (jsonRes.ok) {
          ...
          console.log('JSON 데이터 응답:', jsonData.answer)
        }
