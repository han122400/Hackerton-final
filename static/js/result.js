...
function showToast(message, type = 'success') {
  ...
  toast.className = `fixed bottom-5 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-lg text-sm font-medium z-50 ${
    type === 'success' ? 'bg-green-600 text-white' : 'bg-gray-600 text-white'
  }`
...
function renderResult() {
  ...
  evaluationItems.innerHTML = interviewResult.scores
    .map(
      (score) => `
        ...
        <div class="mb-4">
            ...
            <ul class="mt-2 space-y-1 pl-4">
                ${interviewResult.feedback.positive
                  .map((item) => `<li class="list-disc">${item}</li>`)
                  .join('')}
            </ul>
        </div>
        <div>
            ...
            <ul class="mt-2 space-y-1 pl-4">
                ${interviewResult.feedback.improvement
                  .map((item) => `<li class="list-disc">${item}</li>`)
                  .join('')}
            </ul>
        </div>
  ...
  recommendations.innerHTML = interviewResult.recommendations
    .map(
      (rec, index) => `
        <div class="bg-muted p-3 rounded-lg">
            <span class="font-semibold text-sm block mb-1">추천 ${
              index + 1
            }</span>
